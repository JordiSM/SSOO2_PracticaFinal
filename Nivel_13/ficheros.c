...
int mi_write_f(unsigned int ninodo, const void *buf_original, unsigned int offset, unsigned int nbytes){
    ...
    if(leer_inodo(ninodo, &inodo) == -1){
        fprintf(stderr, "Error leer_inodo mi_write_f %d: %s\n", errno, strerror(errno));
        return -1;
    }
	...
	mi_signalSem();
        ...
        if(bread(nbfisico, buf_bloque) == -1){
            fprintf(stderr, "Error bread mi_write_f %d: %s\n", errno, strerror(errno));
            return -1;
        }
        ...
        if(bwrite(nbfisico, buf_bloque) == -1){
            fprintf(stderr, "Error bwrite en mi_write_f %d: %s\n", errno, strerror(errno));
            return -1;
        }
	...
	mi_waitSem();
        ...
        if(bread(nbfisico, buf_bloque) == -1){
            fprintf(stderr, "Error en bread2 mi_write_f %d: %s\n", errno, strerror(errno));
            return -1;
        }
        ...
        if(bwrite(nbfisico, buf_bloque) == -1){
            fprintf(stderr, "Error en bwrite en mi_write_f %d: %s\n", errno, strerror(errno));
            return -1;
        }
	    ...
	    mi_waitSem();
            ...
            if(bwrite(nbfisico,buf_original + (BLOCKSIZE - desp1) + (i - primerBL - 1) * BLOCKSIZE) == -1){
                fprintf(stderr, "Error en bwrite de mi_write_f %d: %s\n", errno, strerror(errno));
                return -1;
            }
	...
	mi_waitSem();
        ...
        if(bread(nbfisico,buf_bloque) == -1){
            fprintf(stderr, "Error en bread en mi_write_f %d: %s\n", errno, strerror(errno));
            return -1;
        }
    ...
    if(leer_inodo(ninodo, &inodo) == -1){
        fprintf(stderr, "Error leer_inodo en mi_write_f %d: %s\n", errno, strerror(errno));
        mi_signalSem();
        return -1;
    }
    ...
    if(escribir_inodo(ninodo, inodo) == -1){
        fprintf(stderr, "Error en escribir inodo mi_write_f %d: %s\n", errno, strerror(errno));
        mi_signalSem();
        return -1;
    }
...
int mi_chmod_f(unsigned int ninodo, unsigned char permisos){
    ...
    int resultado;
    resultado = escribir_inodo(ninodo, inodo);
    mi_signalSem();
